...
class DT_People_Groups_Base extends DT_Module_Base {
    ...
    public $module = "people_groups_base";
    ...
    public function dt_set_roles_and_permissions( $expected_roles ){
        ...
        if ( !isset( $expected_roles["multiplier"] ) ){
            $expected_roles["multiplier"] = [
                "label" => __( 'Multiplier', 'disciple-tools-people-groups' ),
                "permissions" => []
            ];
        }
        if ( !isset( $expected_roles["dispatcher"] ) ){
            $expected_roles["dispatcher"] = [
                "label" => __( 'Dispatcher', 'disciple-tools-people-groups' ),
        ...
        if ( !isset( $expected_roles["dt_admin"] ) ){
            $expected_roles["dt_admin"] = [
                "label" => __( 'Disciple.Tools Admin', 'disciple-tools-people-groups' ),
                "description" => "All D.T permissions",
                "permissions" => []
            ];
        }
        if ( !isset( $expected_roles["administrator"] ) ){
            $expected_roles["administrator"] = [
                "label" => __( 'Administrator', 'disciple-tools-people-groups' ),
    ...
    public function dt_custom_fields_settings( $fields, $post_type ){
        if ( $post_type === $this->post_type ){
            ...
            $fields["unfollow"] = [
                'name'        => __( 'Un-Follow', 'disciple-tools-people-groups' ),
                'type'        => 'multi_select',
                'default'     => [],
                'hidden'      => true
            ];
            $fields['tasks'] = [
                'name' => __( 'Tasks', 'disciple-tools-people-groups' ),
                'type' => 'post_user_meta',
            ];
            ...
            $fields['assigned_to'] = [
                'name'        => __( 'Assigned To', 'disciple-tools-people-groups' ),
                'description' => __( "Select the main person who is responsible for reporting on this record.", 'disciple-tools-people-groups' ),
                'type'        => 'user_select',
                'default'     => '',
                'tile' => 'status',
            ...
            $fields["requires_update"] = [
                'name'        => __( 'Requires Update', 'disciple-tools-people-groups' ),
                'description' => '',
                'type'        => 'boolean',
                'default'     => false,
            ];
            $fields['status'] = [
                'name'        => __( 'Status', 'disciple-tools-people-groups' ),
                'description' => _x( 'Set the current status.', 'field description', 'disciple-tools-people-groups' ),
                'type'        => 'key_select',
                'default'     => [
                    'inactive' => [
                        'label' => __( 'Inactive', 'disciple-tools-people-groups' ),
                        'description' => _x( 'No longer active.', 'field description', 'disciple-tools-people-groups' ),
                        'color' => "#F43636"
                    ],
                    'active'   => [
                        'label' => __( 'Active', 'disciple-tools-people-groups' ),
                        'description' => _x( 'Is active.', 'field description', 'disciple-tools-people-groups' ),
                        'color' => "#4CAF50"
                    ],
                ],
                'tile'     => '',
            ...
            $fields['contact_count'] = [
                'name' => __( "Contacts Total", 'disciple-tools-people-groups' ),
                'type' => 'number',
                'default' => '0',
                'show_in_table' => true
            ];
            $fields['contacts'] = [
                'name' => __( "Contacts", 'disciple-tools-people-groups' ),
                'type' => 'connection',
                "post_type" => 'contacts',
                'tile' => 'connections',
                "p2p_direction" => "from",
            ...
            $fields['group_total'] = [
                'name' => __( "Groups Total", 'disciple-tools-people-groups' ),
                'type' => 'number',
                'default' => '0',
                'show_in_table' => true
            ];
            $fields['groups'] = [
                'name' => __( "Groups", 'disciple-tools-people-groups' ),
                'type' => 'connection',
                "post_type" => 'groups',
                "p2p_direction" => "from",
            ...
            $fields['location_grid'] = [
                'name'        => __( 'Locations', 'disciple-tools-people-groups' ),
                'description' => _x( 'The general location where this contact is located.', 'Optional Documentation', 'disciple-tools-people-groups' ),
                'type'        => 'location',
                'mapbox'    => false,
                "in_create_form" => true,
                "tile" => "details",
            ...
            $fields['location_grid_meta'] = [
                'name'        => __( 'Locations', 'disciple-tools-people-groups' ), //system string does not need translation
                'description' => _x( 'The general location where this contact is located.', 'Optional Documentation', 'disciple-tools-people-groups' ),
                'type'        => 'location_meta',
                "tile"      => "details",
                'mapbox'    => false,
             ...
             */
            $fields["parents"] = [
                "name" => __( 'Parents', 'disciple-tools-people-groups' ),
                'description' => '',
                "type" => "connection",
            ...
            $fields["peers"] = [
                "name" => __( 'Peers', 'disciple-tools-people-groups' ),
                'description' => '',
                "type" => "connection",
                "post_type" => $this->post_type,
                "p2p_direction" => "any",
            ...
            $fields["children"] = [
                "name" => __( 'Children', 'disciple-tools-people-groups' ),
                'description' => '',
                "type" => "connection",
                "post_type" => $this->post_type,
                "p2p_direction" => "to",
    ...
    public function p2p_init(){
        ...
        p2p_register_connection_type(
            [
                ...
                'title'          => [
                    'from' => __( 'Contacts', 'disciple-tools-people-groups' ),
                    'to'   => $this->plural_name,
                ]
            ]
        );
...
//                'to'          => 'peoplegroups',
//                'title'       => [
//                    'from' => __( 'People Groups', 'disciple-tools-people-groups' ),
//                    'to'   => $this->plural_name,
//                ]
//            ]
//        );
    ...
    public function dt_details_additional_tiles( $tiles, $post_type = "" ){
        if ( $post_type === $this->post_type ){
            $tiles["connections"] = [ "label" => __( "Connections", 'disciple-tools-people-groups' ) ];
            $tiles["other"] = [ "label" => __( "Other", 'disciple-tools-people-groups' ) ];
        }
        return $tiles;
    }
                            ...
                            <div class="typeahead__field">
                                    <span class="typeahead__query">
                                        <input class="js-typeahead-assigned_to input-height"
                                               name="assigned_to[query]" placeholder="<?php echo esc_html_x( "Search Users", 'input field placeholder', 'disciple-tools-people-groups' ) ?>"
                                               autocomplete="off">
                                    </span>
                        ...
                        <div class="typeahead__field">
                            <span class="typeahead__query">
                                <input class="js-typeahead-tags input-height"
                                       name="tags[query]"
                                       placeholder="<?php echo esc_html( sprintf( _x( "Search %s", "Search 'something'", 'disciple-tools-people-groups' ), $fields["tags"]['name'] ) )?>"
                                       autocomplete="off">
                            </span>
                            <span class="typeahead__button">
        ...
        if ( $post_type === $this->post_type && $section === "relationships" ) {
            ...
            <div class="section-subheader members-header" style="padding-top: 10px;">
                <div style="padding-bottom: 5px; margin-right:10px; display: inline-block">
                    <?php esc_html_e( "Member List", 'disciple-tools-people-groups' ) ?>
                </div>
                <button type="button" class="create-new-record" data-connection-key="members" style="height: 36px;">
                    <?php echo esc_html__( 'Create', 'disciple-tools-people-groups' )?>
                ...
                <button type="button"
                    ...
                    <?php echo esc_html__( 'Select', 'disciple-tools-people-groups' )?>
            ...
            <div class="members-section" style="margin-bottom:10px">
                <div id="empty-members-list-message"><?php esc_html_e( "To add new members, click on 'Create' or 'Select'.", 'disciple-tools-people-groups' ) ?></div>
                <div class="member-list">

                </div>
            </div>
            <div class="reveal" id="add-new-group-member-modal" data-reveal style="min-height:500px">
                <h3><?php echo esc_html_x( "Add members from existing contacts", 'Add members modal', 'disciple-tools-people-groups' )?></h3>
                <p><?php echo esc_html_x( "In the 'Member List' field, type the name of an existing contact to add them to this group.", 'Add members modal', 'disciple-tools-people-groups' )?></p>
                ...
                <div class="grid-x pin-to-bottom">
                    ...
                    <button class="button" data-close aria-label="Close reveal" type="button">
                        <?php echo esc_html__( 'Close', 'disciple-tools-people-groups' )?>
                    </button>
                </span>
                    </div>
                </div>
    ...
    public static function dt_user_list_filters( $filters, $post_type ){
        if ( $post_type === self::post_type() ){
            ...
            $filters["tabs"][] = [
                "key" => "assigned_to_me",
                "label" => _x( "Assigned to me", 'List Filters', 'disciple-tools-people-groups' ),
                "count" => $total_my,
                "order" => 20
            ];
            ...
            $filters["filters"][] = [
                'ID' => 'my_all',
                'tab' => 'assigned_to_me',
                'name' => _x( "All", 'List Filters', 'disciple-tools-people-groups' ),
                'query' => [
                    'assigned_to' => [ 'me' ],
                    'sort' => 'status'
                ],
            ...
            $filters["tabs"][] = [
                "key" => "all",
                "label" => _x( "All", 'List Filters', 'disciple-tools-people-groups' ),
                "count" => $total_all,
                "order" => 10
            ];
            // add assigned to me filters
